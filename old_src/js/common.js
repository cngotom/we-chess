...
window.onload = function(){  
	...
	com.get("indexQj").addEventListener("click", function(e) {
        ...
        //网络对战
	com.get("netbatBtn").addEventListener("click", function(e) {
		socket.emit("CCreateRoom",{name:GUserName()});
	});
...
com.resize = function()
{
	var toolbarHeight = 18;
	var screen_width = window.screen.width;
	var screen_height = window.screen.height;

	var scale = screen_width /  (com.stype[com.nowStype].width + 12) ;

	//var offsetY = (screen_height - (com.stype[com.nowStype].height +toolbarHeight)  * scale )/2;

	//com.get("chessBox").style.marginTop= offsetY +"px";
...
com.bylaw.x = function (x,y,map,my){
	var d=[];
	if (my===1&&netplay.isOffensive != false){ //红方
		//4点半
...
com.class.Man = function (key, x, y){
	...
	this.show = function (){
		if (this.isShow) {
			...
			var y = this.y;
			if(netplay.isOffensive == false)
				y = 9 - y;
			
			com.ct.drawImage(com[this.pater].img,com.spaceX * this.x + com.pointStartX , com.spaceY *  y +com.pointStartY);
			com.ct.restore(); 
		}
	}
	
	this.bl = function (map){
		var map = map || play.map || netplay.map
...
com.class.Pane = function (img, x, y){
	...
	this.show = function (){
		if (this.isShow) {
			...
			com.ct.drawImage(com.paneImg, com.spaceX * this.x + com.pointStartX , com.spaceY *  y + com.pointStartY)
			com.ct.drawImage(com.paneImg, com.spaceX * this.newX + com.pointStartX  , com.spaceY *  newY + com.pointStartY)
		}
	}
}
...
com.class.Dot = function (img, x, y){
	...
	this.show = function (){
		for (var i=0; i<this.dots.length;i++){
			...
			if (this.isShow) com.ct.drawImage(com.dotImg, com.spaceX * this.dots[i][0]+10  + com.pointStartX ,com.spaceY *  y+10 + com.pointStartY) }
	}
}
